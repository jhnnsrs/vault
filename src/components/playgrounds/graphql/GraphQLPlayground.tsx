...
import {  Table, Window, Book } from 'react-bootstrap-icons';
import { gql, rewriteURIForGET, useApolloClient } from "@apollo/client";
...
export const UnconnectedGraphQLPlayground = ({initialQuery, height, width}) => {
    ...
    return (<div style={{width: width || "100%", height: height || 600, marginBottom: "10px"}} className="relative flex">
        <div className="graphql-playground flex-grow"></div>
        <GraphiQL 
        ref={graphiql}
        fetcher={subObservableFetcher} editorTheme={'duotone-dark'} query={query} onEditQuery={handleEditQuery} schema={schema}>
            <GraphiQL.Toolbar>
                ...
                <button id="excel" onClick={() => handleToggleExploer()} className="excel" title="Open in New Window" aria-label="Open Explorer"><Book color="white" size={17} /></button>
            </GraphiQL.Toolbar>
