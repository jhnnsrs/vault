...
export class AuthService<TIDToken = JWTIDToken> {
  ...
  constructor(props: AuthServiceProps, window: any) {
    this.props = props;
    this.window = window
  ...
  setAuthTokens(auth: AuthTokens): void {
    ...
    this.window.localStorage.setItem("auth", JSON.stringify(auth));
  }
  ...
  authorize(): boolean {
    ...
    this.window.localStorage.setItem("preAuthUri", window.location.href);
    this.window.localStorage.removeItem("auth");
